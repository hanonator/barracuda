...
public class AnnotatedTypeImpl<X> extends AbstractAnnotated implements AnnotatedType<X> {
	 ...
	 *            class
	 */
	public AnnotatedTypeImpl(Class<X> annotatedClass, AnnotatedType<? super X> supertype, BeanManager bm) {
		...
		if (supertype == null) {
			...
			for (Annotation annotation : supertype.getAnnotations()) {
				...
				if (annotationType.isAnnotationPresent(Inherited.class) && !annotationTypes.contains(annotationType) && (!bm.isScope(annotationType) || !hasScope)) {
					annotations.add(annotation);
	...
	private class State {
		...
		private State() {
			Constructor<?>[] decCtxs = annotatedClass.getConstructors();
			...
			for (Constructor<?> ct : decCtxs) {
				if (!ct.isSynthetic()) {
					AnnotatedConstructor<X> ac = new AnnotatedConstructorImpl<X>((Constructor<X>) ct, AnnotatedTypeImpl.this);
					constructors.add(ac);
				}
			}
			if (constructors.isEmpty()) {
				try {
					...
					Constructor<X> constructor = annotatedClass.getDeclaredConstructor();
					if (constructor != null) {
						constructors.add(new AnnotatedConstructorImpl<X>(constructor, AnnotatedTypeImpl.this));
					}	
				} catch (Exception ex) {
					ex.printStackTrace();
				}
			...
			Field[] decFields = annotatedClass.getDeclaredFields();
			Method[] decMethods = annotatedClass.getDeclaredMethods();
			for (Field f : decFields) {
				if (!f.isSynthetic()) {
					AnnotatedField<X> af = new AnnotatedFieldImpl<X>(f, AnnotatedTypeImpl.this);
					fields.add(af);
				}
			}
			...
			for (Method m : decMethods) {
				if (!m.isSynthetic() && !m.isBridge()) {
					AnnotatedMethod<X> am = new AnnotatedMethodImpl<X>(m, AnnotatedTypeImpl.this);
					methods.add(am);
				}
			}
			...
			if (supertype != null) {
				for (AnnotatedField<? super X> field : supertype.getFields()) {
					fields.add(new AnnotatedFieldImpl<X>(field.getJavaMember(), AnnotatedTypeImpl.this));
				}
				for (AnnotatedMethod<? super X> method : supertype.getMethods()) {
					methods.add(new AnnotatedMethodImpl<X>(method.getJavaMember(), AnnotatedTypeImpl.this));
				}
			}
